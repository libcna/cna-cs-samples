...
- The content must be **official XNA Content Pipeline output**. `../cna-samples/samples/<Port>/Content/`
  is normally where it lives: those trees were generated by the official pipeline and verified
  byte-identical against it by that campaign, so re-deriving them here would add risk and prove
  nothing.
